...
import com.fc.common.shiro.realm.OAuthToken;
/**
 * @author 科技㊣²º¹³
 * 2015年7月8日 下午4:48:45
 * http://www.rekoe.com
...
public class OauthActtion {
	...
	public View returnPoint(String providerId, HttpServletRequest request, HttpSession session) throws Exception {
		...
		try {
			...
			return new ViewWrapper(new ForwardView("/private/ologin"), "帐号不存在");
		} catch (IncorrectCredentialsException ice) {
			return new ViewWrapper(new ForwardView("/private/ologin"), "证书验证失败");
		} catch (LockedAccountException lae) {
			return new ViewWrapper(new ForwardView("/private/ologin"), "帐号已被锁定");
		} catch (ExcessiveAttemptsException eae) {
			return new ViewWrapper(new ForwardView("/private/ologin"), "尝试的次数太多");
		} catch (AuthenticationException ae) {
			return new ViewWrapper(new ForwardView("/private/ologin"), ae.getMessage());
		}
		if (rname) {
			return new ViewWrapper(new ServerRedirectView("/private/register"), null);
		}
		return new ViewWrapper(new ServerRedirectView("/private/index"), null);
	}
